    ...
    GstCaps * outcaps)
{
  ...
  if (priv->mask_pool == NULL) {
    ...
    priv->mask_pool = gst_video_buffer_pool_new ();
    GstStructure *config = gst_buffer_pool_get_config (priv->mask_pool);
    gst_buffer_pool_config_set_params (config, caps, priv->mask_length, 0, 0);
    gst_buffer_pool_config_add_option (config,
        GST_BUFFER_POOL_OPTION_VIDEO_META);
    g_return_val_if_fail (gst_buffer_pool_set_config (priv->mask_pool, config),
        FALSE);
    g_return_val_if_fail (gst_buffer_pool_set_active (priv->mask_pool, TRUE),
        FALSE);
    gst_caps_unref (caps);
  }
...
#define BB_H(candidate) candidate[h_offset]
  ...
  for (gsize c = 0; c < sel_candidates->len; c++) {
    ...
    if (keep) {
      ...
      g_return_if_fail (GST_IS_BUFFER (mask_buf));
      GstVideoMeta *vmeta = gst_buffer_get_video_meta (mask_buf);
      g_return_if_fail (vmeta != NULL);
