...
typedef struct _GstH264LevelLimit GstH264LevelLimit;

struct _GstH264LevelLimit
{
  const gchar *name;
...
gst_h264_encoder_reset (GstVideoEncoder * encoder, gboolean hard)
{
  ...
  /* TODO: Add a proper management of rate control in the base class, see vah264enc */
  priv->rc.max_bitrate = 0;
  priv->rc.max_bitrate_bits = 0;
  priv->rc.cpb_length_bits = 0;
    ...
    GstH264EncodeFrame * h264_frame)
{
  ...
  GST_INFO_OBJECT (self,
      "%" G_GINT64_FORMAT
      " frames have been proceed for a total used bytes of %" G_GINT64_FORMAT,
      priv->nb_frames, priv->used_bytes);
}
...
_find_unused_reference_frame (GstH264Encoder * self, GstH264EncodeFrame * frame)
{
  ...
  for (i = 0; i < g_queue_get_length (&priv->ref_list); i++) {
    GstH264EncodeFrame *h264f;
    GstVideoCodecFrame *f;

    f = g_queue_peek_nth (&priv->ref_list, i);
    h264f = _enc_frame (f);
    if (h264f->type != GST_H264_B_SLICE)
      continue;
    ...
    b_h264frame = _enc_frame (b_frame);
    g_assert (h264f->poc != b_h264frame->poc);
    if (h264f->poc < b_h264frame->poc) {
      b_frame = f;
      b_h264frame = _enc_frame (b_frame);
    }
  }
...
_encode_one_frame (GstH264Encoder * self, GstVideoCodecFrame * gst_frame)
{
  ...
  if (h264_frame->type != GST_H264_I_SLICE) {
    ...
    for (i = g_queue_get_length (&priv->ref_list) - 1; i >= 0; i--) {
      f = g_queue_peek_nth (&priv->ref_list, i);
      h264f = _enc_frame (f);
      if (h264f->poc > h264_frame->poc)
        continue;

      list0[list0_num] = h264f;
      list0_num++;
    }
...
gst_h264_encoder_class_init (GstH264EncoderClass * klass)
{
  ...
  g_object_class_install_property (object_class, PROP_BFRAMES,
      g_param_spec_uint ("b-frames", "B Frames",
          "Number of consecutive B frames between I and P reference frames", 0,
          31, 0,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_CONSTRUCT));
}
...
gst_h264_encoder_get_level_limit (GstH264Encoder * encoder)
{
  ...
  const guint mbWidth = GST_ROUND_UP_16 (priv->width) / 16;
  const guint mbHeight = GST_ROUND_UP_16 (priv->height) / 16;
...
/**
 * gst_h264_encoder_get_gop_size:
 * @encoder: a #GstH264Encoder
 *
 * Get the current gop size
 *
 * Returns: (transfer full): a gop size.
 *
 * Since: 1.26
 */
guint32
gst_h264_encoder_get_gop_size (GstH264Encoder * encoder)
{
  guint32 idr_period = 0;
  GstH264EncoderPrivate *priv = gst_h264_encoder_get_instance_private (encoder);

  GST_VIDEO_ENCODER_STREAM_LOCK (encoder);
  idr_period = priv->gop.idr_period;
  GST_VIDEO_ENCODER_STREAM_UNLOCK (encoder);
...
/**
 * gst_h264_encoder_get_gop_b_frames_count:
 * @encoder: a #GstH264Encoder
 *
 * Get consecutive b frames count
 *
 * Returns: (transfer full): a b frames count.
 *
 * Since: 1.26
 */
guint32
gst_h264_encoder_get_gop_b_frames_count (GstH264Encoder * encoder)
{
  guint32 num_bframes = 0;
  GstH264EncoderPrivate *priv = gst_h264_encoder_get_instance_private (encoder);

  GST_VIDEO_ENCODER_STREAM_LOCK (encoder);
  num_bframes = priv->gop.num_bframes;
  GST_VIDEO_ENCODER_STREAM_UNLOCK (encoder);
