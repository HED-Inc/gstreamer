...
typedef struct _H264GOPFrame H264GOPFrame;
struct _H264GOPFrame
{
  guint8 slice_type;
  gboolean is_ref;
...
struct _GstH264EncoderClass
{
  ...
  gboolean (*set_format)        (GstH264Encoder * encoder,
                                 GstVideoCodecState * state);

  gboolean (*max_num_reference) (GstH264Encoder * encoder,
                                 guint32 * max_l0_reference_count,
                                 guint32 * max_l1_reference_count);
  /*< private > */
  gpointer padding[GST_PADDING_LARGE];
};

gboolean             gst_h264_encoder_reset                  (GstVideoEncoder * encoder,
                                                              gboolean hard);

GstH264Level         gst_h264_encoder_get_level_limit        (GstH264Encoder * encoder);

void                 gst_h264_encoder_set_profile            (GstH264Encoder * encoder,
...
guint32              gst_h264_encoder_get_gop_size           (GstH264Encoder * encoder);
guint32              gst_h264_encoder_get_gop_b_frames_count (GstH264Encoder * encoder);
