...
typedef struct _VkH264Params VkH264Params;
...
gst_vulkan_h264_encoder_init_std_sps (GstVulkanH264Encoder * self, gint sps_id)
{
  ...
  self->session_params.sps = (StdVideoH264SequenceParameterSet) {
    ...
    .level_idc = gst_vulkan_h264_level_idc (gst_h264_encoder_get_level_limit (base)),
    .chroma_format_idc =
    ...
    guint list0_num, GstH264EncodeFrame ** list1, guint list1_num)
{
  ...
  if (self->prop.aud && !_add_aud (self, h264_frame)) {
    GST_ERROR_OBJECT (self, "Unable to add AUD");
    return GST_FLOW_ERROR;
  }

  /* Repeat the SPS for IDR. */
  if (h264_frame->poc == 0 && !_add_vulkan_params_header (self, vk_frame)) {
    GST_ERROR_OBJECT (self, "Unable to add params header");
    return GST_FLOW_ERROR;
  }
  ...
  if (self->prop.rate_ctrl !=
      VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR) {
    vk_frame->slice_info.constantQp = DEFAULT_H264_QP_MIN;
  }
  ...
  vk_frame->rc_info = (VkVideoEncodeH264RateControlInfoKHR) {
    ...
    .pNext = NULL,
    .gopFrameCount = gst_h264_encoder_get_gop_size (base),
    .idrPeriod = gst_h264_encoder_get_gop_size (base),
    .consecutiveBFrameCount = gst_h264_encoder_get_gop_b_frames_count (base),
