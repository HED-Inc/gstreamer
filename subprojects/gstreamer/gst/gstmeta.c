...
gst_meta_api_type_aggregate_params (GType api,
    GstStructure ** aggregated_params, GstStructure * params0,
    GstStructure * params1)
{
  ...
  GstAllocationMetaParamsAggregator aggregator_func =
      g_type_get_qdata (api, GST_QUARK (ALLOCATION_META_PARAMS_AGGREGATOR));
...
/**
 * gst_meta_api_type_set_aggregator:
 ...
 */
void
gst_meta_api_type_set_aggregator (GType api,
    GstAllocationMetaParamsAggregator aggregator)
{
  g_return_if_fail (api != 0);
  g_return_if_fail (aggregator != NULL);

  g_type_set_qdata (api, GST_QUARK (ALLOCATION_META_PARAMS_AGGREGATOR),
      (GstAllocationMetaParamsAggregator) aggregator);
}
